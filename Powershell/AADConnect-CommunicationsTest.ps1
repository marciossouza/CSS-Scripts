...
.DESCRIPTION AAD Communications Test.
...
- Specified DCs for connectivity on ports 135, 445, 389, and 3268
- Ability to download CRL on port 80
- Access to AAD connect endpoints on port 443
- Access to AAD Health / service bus endpoints on 5671
- Report on general system prerequisite/configuration settings
...
function AzureCredential
{
    ...
    If (Get-Module AzureAD -ListAvailable -ea silentlycontinue)
    { try
        {
            Import-Module AzureAD
            $AADLogonResult = Connect-AzureAD $Credential -ea Stop
